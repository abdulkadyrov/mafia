...
server.listen(port, "0.0.0.0", () => {
  ...
  console.log(
    "Open one Network URL on every phone. VPN and iCloud Private Relay should be disabled."
  );
...
server.on("error", (error) => {
  if (error && error.code === "EADDRINUSE") {
    console.error(
      `Port ${port} is already busy. Try: PORT=${port + 1} npm run lan`
    );
    process.exit(1);
  }

  if (error && error.code === "EACCES") {
    console.error(`Port ${port} is not allowed. Try: PORT=4174 npm run lan`);
    process.exit(1);
  }
