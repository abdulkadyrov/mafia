...
export function seedDemoPlayers(
  ...
  const needed = Math.max(
    0,
    snapshot.settings.playerLimit - snapshot.players.length
  );
...
export function startGame(snapshot: GameSnapshot): GameSnapshot {
  const normalizedRoles = normalizeRoleSettings(
    snapshot.settings.roles,
    snapshot.players.length
  );
  const manualRoles = normalizeRoleSettings(
    countRoles(snapshot.players),
    snapshot.players.length
  );
  const shouldUseManualRoles =
    snapshot.settings.roleAssignmentMode === "manual";
  const players = shouldUseManualRoles
    ? resetPlayersForGame(snapshot.players)
    : assignRandomRoles(snapshot.players, normalizedRoles);
  ...
  return enterPhase(
    addSystemMessage(startedSnapshot, "Наступила ночь"),
    "Night"
  );
}
...
export function enterPhase(
  ...
  const phaseSeconds = timerKey
    ? snapshot.settings.timers[timerKey]
    : undefined;
  ...
  return {
    ...
    phase,
    phaseStartedAt,
    phaseEndsAt: phaseSeconds
      ? phaseStartedAt + phaseSeconds * 1000
      : undefined,
  };
}
  ...
  if (action.type === "doctorHeal" && actor.id === target.id) {
    if (actor.selfHealsUsed >= snapshot.settings.doctorSelfHealsLimit) {
      return addPrivateHistory(
        snapshot,
        actor.id,
        "Лимит самолечения уже использован"
      );
    }
  }

  const withoutPreviousAction = snapshot.nightActions.filter(
    (existingAction) => {
      return (
        existingAction.actorId !== action.actorId ||
...
export function resolveNight(snapshot: GameSnapshot): GameSnapshot {
  const healedPlayerIds = new Set(
    snapshot.nightActions
      .filter((action) => action.type === "doctorHeal")
      .map((action) => action.targetId)
  );

  const killActions = [
    ...getResolvedMafiaKillActions(snapshot),
    ...snapshot.nightActions.filter(
      (action) => action.type === "detectiveKill"
    ),
  ];
  ...
  for (const action of killActions) {
    ...
    const alreadyDead = deaths.some(
      (death) => death.playerId === action.targetId
    );
    ...
    deaths.push({
      ...
      text:
        action.type === "mafiaKill" ? "Тебя убила мафия" : "Тебя убил детектив",
    });
  }
  ...
  const players = snapshot.players.map((player) => {
    ...
    return {
      ...
      selfHealsUsed: usedSelfHeal
        ? player.selfHealsUsed + 1
        : player.selfHealsUsed,
      score: calculateNightScore(
        player,
        deaths,
        savedPlayerIds,
        snapshot.nightActions,
...
export function submitVote(snapshot: GameSnapshot, vote: Vote): GameSnapshot {
  ...
  const votes = snapshot.votes.filter(
    (existingVote) => existingVote.voterId !== vote.voterId
  );
...
export function resolveVotes(snapshot: GameSnapshot): GameSnapshot {
  const votesByTarget = snapshot.votes.reduce<Record<PlayerId, number>>(
    (accumulator, vote) => {
  ...
  return enterPhase(
    {
      ...addSystemMessage(
        snapshot,
        "Город переходит к подтверждению исключения"
      ),
      confirmationVotes: [],
      lastVoteResolution: resolution,
    },
...
export function submitConfirmationVote(
  snapshot: GameSnapshot,
  ...
  const votes = snapshot.confirmationVotes.filter((existingVote) => {
    return (
      existingVote.voterId !== vote.voterId ||
      existingVote.targetId !== vote.targetId
    );
  });
...
export function resolveConfirmation(snapshot: GameSnapshot): GameSnapshot {
  const candidates =
    snapshot.lastVoteResolution?.confirmationCandidateIds ?? [];
  const confirmationVotesByTarget = candidates.reduce<
    Record<PlayerId, { exclude: number; keep: number }>
  ...
  const players = snapshot.players.map((player) => {
    ...
    const betDelta =
      snapshot.settings.bettingMode && vote
        ? guessedCorrectly
          ? vote.bet
          : -vote.bet
        : 0;
  ...
  if (eliminatedPlayerId) {
    ...
    resolvedSnapshot = addSystemMessage(
      resolvedSnapshot,
      `${eliminatedPlayer?.name ?? "Игрок"} был изгнан`
    );
  } else {
    resolvedSnapshot = addSystemMessage(
      resolvedSnapshot,
      "Город не смог выбрать цель"
    );
  }
...
export function moveToVoting(snapshot: GameSnapshot): GameSnapshot {
  return enterPhase(
    addSystemMessage(snapshot, "Началось голосование"),
    "Voting"
  );
}
...
export function toClientSnapshot(
  ...
  const showRoles =
    snapshot.phase === "GameOver" || snapshot.settings.revealRolesAfterDeath;
...
function calculateNightScore(
  ...
  const savedSomeone = nightActions.some((action) => {
    return (
      action.actorId === player.id &&
      action.type === "doctorHeal" &&
      savedPlayerIds.includes(action.targetId)
    );
  });
  ...
  const mafiaKilled = nightActions.some((action) => {
    return (
      action.actorId === player.id &&
      action.type === "mafiaKill" &&
      deaths.some((death) => death.playerId === action.targetId)
    );
  });
  ...
  const detectiveTargetDeath = detectiveKill
    ? deaths.find(
        (death) =>
          death.playerId === detectiveKill.targetId &&
          death.killerId === player.id
      )
    : undefined;
  ...
  if (detectiveTargetDeath) {
    const killedPlayer = snapshot.players.find(
      (target) => target.id === detectiveTargetDeath.playerId
    );
    const knewTarget = (snapshot.detectiveKnowledge[player.id] ?? []).includes(
      detectiveTargetDeath.playerId
    );
...
function getResolvedMafiaKillActions(snapshot: GameSnapshot): NightAction[] {
  ...
  const aliveMafiaIds = snapshot.players
    .filter((player) => player.alive && player.role === "mafia")
    .map((player) => player.id);
  const votesByTarget = mafiaActions.reduce<Record<PlayerId, NightAction[]>>(
    (accumulator, action) => {
      accumulator[action.targetId] = [
        ...(accumulator[action.targetId] ?? []),
        action,
      ];
      return accumulator;
  ...
  if (snapshot.settings.mafiaDecisionMode === "unanimity") {
    const unanimousTarget = Object.entries(votesByTarget).find(
      ([, actions]) => actions.length === aliveMafiaIds.length
    );
    return unanimousTarget ? [unanimousTarget[1][0]] : [];
  }

  const [targetActions] = Object.values(votesByTarget).sort(
...
function buildDetectiveKnowledge(
  ...
  const nextKnowledge: Record<PlayerId, PlayerId[]> = {
    ...snapshot.detectiveKnowledge,
  };

  for (const check of checks) {
    nextKnowledge[check.detectiveId] = [
      ...new Set([...(nextKnowledge[check.detectiveId] ?? []), check.targetId]),
    ];
  }
...
function addNightPublicMessages(
  snapshot: GameSnapshot,
  ...
  for (const death of resolution.deaths) {
    ...
    nextSnapshot = addSystemMessage(
      nextSnapshot,
      `${player?.name ?? "Игрок"} был убит`
    );
  }
  ...
  for (const savedPlayerId of resolution.savedPlayerIds) {
    ...
    nextSnapshot = addSystemMessage(
      nextSnapshot,
      `Доктор спас ${player?.name ?? "игрока"}`
    );
  }

  if (
    resolution.deaths.length === 0 &&
    resolution.savedPlayerIds.length === 0
  ) {
...
function addDetectivePrivateMessages(
  snapshot: GameSnapshot,
  resolution: NightResolution
): GameSnapshot {
  return resolution.detectiveChecks.reduce((currentSnapshot, check) => {
    const target = snapshot.players.find(
      (player) => player.id === check.targetId
    );
...
function finishGame(snapshot: GameSnapshot): GameSnapshot {
  const mvp = [...snapshot.players].sort(
